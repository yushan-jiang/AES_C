...
int keyExpansion(const uint8_t *key, uint32_t keyLen, AesKey *aesKey) {
    ...
    /* W[4-43] */
    for (int i = 0; i < 10; ++i) {
        w[4] = w[0] ^ MIX(w[3]) ^ rcon[i]; //rcon 轮常量表
        w[5] = w[1] ^ w[4];
        w[6] = w[2] ^ w[5];
        w[7] = w[3] ^ w[6];
        w += 4;
...
uint8_t GMul(uint8_t u, uint8_t v) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        // a7=1 ? v左移与1B异或 : v左移
        int flag = (v & 0x80); // 判断：v的最高位是否为1
        v <<= 1;
        if (flag) {
            v ^= 0x1B;   /* GF(256)的本原多项式：x^8 + x^4 + x^3 + x + 1 */
        }
        u >>= 1;
    }
    return p;
...
int aesEncrypt(const uint8_t *key, uint32_t keyLen, const uint8_t *pt, uint8_t *ct, uint32_t len) {
    ...
    memcpy(actualKey, key, keyLen);  // key -> actualkey
